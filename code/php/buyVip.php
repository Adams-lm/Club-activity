<?php
    ...
    include("conn.php");
    //判断用户余额是否充足
    $sql1="SELECT (SELECT price from vip where type = ?) <= balance FROM user where user_id = ?;";
    $stmt1=mysqli_prepare($conn,$sql1);
    mysqli_stmt_bind_param($stmt1,"ii",$type,$userId);
    ...
    if($row3=mysqli_fetch_array($rs3)){
        ...
        if($is_past){
            ...
            if(mysqli_affected_rows($conn)>0){
                page_redirect(1,NULL,"成功开通"."$type"."个月VIP会员！");
            }
            else{
                page_redirect(1,NULL,"开通会员失败！！！");
            }
        }
        //没有过期
        else{
            ...
            mysqli_stmt_execute($stmt);
            if(mysqli_affected_rows($conn)>0){
                page_redirect(1,NULL,"成功续费"."$type"."个月VIP会员!");
            }
            else{
                page_redirect(1,NULL,"续费会员失败！！！");
            }
        }
    }
    else{
        ...
        mysqli_stmt_execute($stmt);
        if(mysqli_affected_rows($conn)>0){
            page_redirect(1,NULL,"成功首次开通"."$type"."个月VIP会员!");
        }
        else{
            page_redirect(1,NULL,"开通会员失败！！！");
        }
